%{
	...
	int branch_pomocna_promenljiva;
%}
...
branch_statement
	: _BRANCH _LPAREN _ID
		{
			if (($<i>$ = lookup_symbol($3, VAR|PAR)) == NO_INDEX)
				err("'%s' is undeclared", $3);
		}
	_SEMICOLON literal 
		{
			if (get_type($<i>4) != get_type($6))
				err("incompatible types...");
			
			branch_pomocna_promenljiva = atoi(get_name($6));
		}
	_COMMA literal 
		{
			if (get_type($<i>4) != get_type($9))
				err("incompatible types...");
			
			int pom = atoi(get_name($9));
			if (pom < branch_pomocna_promenljiva)
				err("const2 must be greater than const1");
			
			branch_pomocna_promenljiva = pom;
		}
	_COMMA literal 
		{
			if (get_type($<i>4) != get_type($12))
				err("incompatible types...");
